...
import warnings
from dataloader import SRNDataset
...
parser.add_argument('--train_dir', dest='train_dir', default='/public/home/wangchy5/CV2-Final/GAN/pi-GAN/SRN/cars_train', help='training data dir')
parser.add_argument('--checkpoint', dest='checkpoint', default='/public/home/wangchy5/SRNVI-GAN/checkpoint/9000.pth', help='checkpoint path')
parser.add_argument('--save_frequency', dest='save_frequency', default=1000, type=int,
                    help='save model every # of iteration')
parser.add_argument('--print_frequency', dest='print_frequency', default=10, type=int,
                    help='print loss every # of iteration')
parser.add_argument('--multi_GPU', dest='multi_GPU', default=False, type=bool, help='use multi GPU or not')
...
parser.add_argument('--lambda_cyc_VGG', type=float, default=1.0, help='weight of cycle perceptual loss')
parser.add_argument('--lambda_VI', type=float, default=1.0, help='weight of view independent loss')
parser.add_argument('--lambda_G', type=float, default=1.0, help='weigth of GAN loss')
parser.add_argument('--lambda_pose', type=float, default=1.0, help='weigth of pose prediction loss')
...
if __name__ == "__main__":
    ...
    writer = SummaryWriter(log_dir='log_50' + os.path.basename(args.train_dir))
    ...
    # train or continue
    if args.train == 1:
        dataset = SRNDataset("train", "/public/home/wangchy5/CV2-Final/GAN/pi-GAN/SRN/SRN/cars_train", "/public/home/wangchy5/CV2-Final/data/cars.pickle", imgsize=128)
    ...
    if args.train == 2:
        ...
        dataset = SRNDataset("train", "/public/home/wangchy5/CV2-Final/GAN/pi-GAN/SRN/SRN/cars_train",
                             "/public/home/wangchy5/CV2-Final/data/cars.pickle", imgsize=128)
        dataloader = DataLoader(dataset, batch_size=args.batch_size, shuffle=True)
        iterations = int(len(dataset) / args.batch_size)
    ...
    for e in range(epoch, args.epoch):
        ...
        # for i in range(it, iterations):
            ...
            if (e * iterations + i) % args.print_frequency == args.print_frequency - 1:
                ...
                print("d_loss:", "{:.2f}".format(d_loss.item()), "g_loss:", "{:.2f}".format(g_loss.item()),
                      "d_Pose_loss", "{:.2f}".format(loss_D_P.item()),
                      ...
                      "cyc_per:", "{:.2f}".format(cyc_per_loss.item()), "recons:",
                      "{:.2f}".format(reconstruction_loss.item()),
            ...
            if (e * iterations + i) % args.save_frequency == args.save_frequency - 1:
                torch.save({
                    'epoch': e,
                    'iteration': i,
                    'encoder_state': encoder.state_dict(), 'decoder_state': decoder.state_dict(),
                ...
                filename = os.path.join(figure_dir, f'Testing_results_{e * iterations + i + 1}.png')

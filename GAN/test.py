...
parser.add_argument('--test_dir', dest='test_dir', default='/public/home/wangchy5/CV2-Final/GAN/pi-GAN/SRN/cars_train', help='testing data dir')
parser.add_argument('--checkpoint', dest='checkpoint', default='/public/home/wangchy5/SRNVI-GAN/checkpoint/24000.pth', help='checkpoint path')
...
if __name__ == "__main__":
    ...
    for i, (imgs, poses) in enumerate(dataloader):
        ...
        output_path ="./ouptut/final/" + name + "output.png"
        ...
        one = Image.fromarray(tensor2im(imgA2B).astype('uint8'), 'RGB')
        two = Image.fromarray(tensor2im(img2).astype('uint8'), 'RGB')
        zero = Image.fromarray(tensor2im(img1).astype('uint8'), 'RGB')
        fig, axes = plt.subplots(1, 3)
        axes[1].imshow(one)
        axes[1].axis('off')
        axes[1].text(0.5, -0.1, 'Image AtoB', transform=axes[1].transAxes,
                     ha='center', va='center', fontsize=12, color='red')
        axes[2].imshow(two)
        axes[2].axis('off')
        axes[2].text(0.5, -0.1, 'Image B', transform=axes[2].transAxes,

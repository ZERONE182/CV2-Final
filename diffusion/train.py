...
for e in range(100000):
    ...
    for img, R, T, K in tqdm(loader):
        ...
        loss = p_losses(model, img=img.cuda(), R=R.cuda(), T=T.cuda(), K=K.cuda(), logsnr=logsnr.cuda(), loss_type="l2",
                        cond_prob=0.1)
        loss.backward()
        optimizer.step()
        ...
        if step % 1000 == 900:
            ...
            with torch.no_grad():
                for oriimg, R, T, K in loader_val:
                    ...
                    img = rearrange(((img[-1].clip(-1, 1) + 1) * 127.5).astype(np.uint8), "(b a) c h w -> a c h (b w)",
                                    a=8, b=16)

                    gt = rearrange(((oriimg[:, 1] + 1) * 127.5).detach().cpu().numpy().astype(np.uint8),
                                   "(b a) c h w -> a c h (b w)", a=8, b=16)
                    cd = rearrange(((oriimg[:, 0] + 1) * 127.5).detach().cpu().numpy().astype(np.uint8),
                                   "(b a) c h w -> a c h (b w)", a=8, b=16)
